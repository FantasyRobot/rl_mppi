...
class T12A14MuJoCoEnvConstraints:
    """MuJoCo env wrapper for t12a_14 with explicit joint velocity + acceleration constraints.

    This mirrors the idea in env/envball_constraints.py:
    - Report constraint violation signals via info (binary + amount)
    - Enforce hard bounds by clamping *joint velocities* after each physics step

    Constraints (per actuated DoF i):
    - |qvel_i| <= vel_bound
    - |qacc_i| <= acc_bound, where qacc is approximated as finite-diff of qvel over dt

    Notes:
    - We clamp qvel in-place after each mujoco step. This is a pragmatic safety clamp
      for learning; it is not identical to a physically exact constrained simulation.
    - Observation and action conventions match T12A14MuJoCoEnv.
    """
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            raise ModuleNotFoundError("T12A14MuJoCoEnvConstraints requires 'mujoco' (pip install mujoco).") from e
        ...
        self.violation_tol = float(max(0.0, violation_tol))
        self.violation_agg = str(violation_agg).strip().lower()
        if self.violation_agg not in {"max", "sum"}:
            raise ValueError(f"Unknown violation_agg: {violation_agg} (expected: max|sum)")
    ...
    def step(self, action_norm: np.ndarray) -> tuple[np.ndarray, float, bool, dict]:
        ...
        # Aggregate violation across action_repeat.
        # - 'max': robust, scale-stable, good for learning signals
        # - 'sum': more punitive, but can saturate easily for high-DoF systems
        vel_violation_amount = 0.0
        acc_violation_amount = 0.0
        constraint_violation = False

        # Only constrain the first nu components (actuated DoFs) for qvel/qacc.
        qsl = slice(0, int(self.nu))

        for _ in range(self.action_repeat):
            qvel_prev = np.asarray(self.data.qvel[qsl], dtype=np.float64).copy()

            self.data.ctrl[:] = ctrl
            self._mujoco.mj_step(self.model, self.data)

            qvel_raw = np.asarray(self.data.qvel[qsl], dtype=np.float64).copy()

            # Velocity violation from raw post-step velocity.
            v_over = np.maximum(0.0, np.abs(qvel_raw) - float(self.vel_bound))
            v_amt = float(np.max(v_over)) if self.violation_agg == "max" else float(np.sum(v_over))

            # Acceleration violation from raw delta velocity.
            dq = qvel_raw - qvel_prev
            dt = float(self._dt) if float(self._dt) > 0 else 1.0
            qacc_raw = dq / dt
            a_over = np.maximum(0.0, np.abs(qacc_raw) - float(self.acc_bound))
            a_amt = float(np.max(a_over)) if self.violation_agg == "max" else float(np.sum(a_over))

            if v_amt > float(self.violation_tol) or a_amt > float(self.violation_tol):
                constraint_violation = True

            if self.violation_agg == "max":
                vel_violation_amount = max(float(vel_violation_amount), float(v_amt))
                acc_violation_amount = max(float(acc_violation_amount), float(a_amt))
            else:
                vel_violation_amount += float(v_amt)
                acc_violation_amount += float(a_amt)

            # Enforce acceleration: clamp delta qvel.
            dq_clipped = np.clip(dq, -float(self.acc_bound) * dt, float(self.acc_bound) * dt)
            qvel_acc_limited = qvel_prev + dq_clipped

            # Enforce velocity bound.
            qvel_clipped = np.clip(qvel_acc_limited, -float(self.vel_bound), float(self.vel_bound))

            # Apply clamp for the next physics step.
            self.data.qvel[qsl] = qvel_clipped
        ...
        info = {
            ...
            "acc_violation_amount": float(acc_violation_amount),
            "constraint_violation_amount": float(vel_violation_amount + acc_violation_amount),
            "violation_tol": float(self.violation_tol),
            "violation_agg": str(self.violation_agg),
            "qpos": np.asarray(self.data.qpos[qsl], dtype=np.float32).copy(),
            "qvel": np.asarray(self.data.qvel[qsl], dtype=np.float32).copy(),
            "vel_bound": float(self.vel_bound),
