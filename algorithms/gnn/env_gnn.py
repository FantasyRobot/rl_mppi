...
class MultiRobotGNNEnv:
    ...
    def get_obs(self):
        ...
        self._update_goal()
        # 归一化goal到[0,1]（假设工作空间为[0.3,0.8],[0,0.4],[0.2,0.5]）
        goal_low = np.array([0.3, 0.0, 0.2], dtype=np.float32)
        goal_high = np.array([0.8, 0.4, 0.5], dtype=np.float32)
        
        goal_norm = (self.goal_pos.astype(np.float32) - goal_low) / (goal_high - goal_low + 1e-8)
        goal_pose = np.eye(4, dtype=np.float32)
        goal_pose[:3, 3] = goal_norm
        targets = {
    ...
    def sac_get_obs(self) -> np.ndarray:
    ...
    def sac_reset(self, init_qpos=None) -> np.ndarray:
        self._step_count = 0
        self.data.qvel[:] = 0.0
        if init_qpos is not None:
    ...
    def sac_step(self, action_norm: np.ndarray) -> tuple[np.ndarray, float, bool, dict]:
