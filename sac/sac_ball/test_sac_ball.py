...
from sac_utils import SACAgent, scale_state
...
def test_sac_ball(
    ...
    for test_idx in range(num_tests):
        ...
        # Debug输出动作范围
        action_debug = np.array(action_debug)
        print(f"Test {test_idx + 1} action range: min {action_debug.min(axis=0)}, max {action_debug.max(axis=0)}")
...
def main() -> None:
    ...
    parser.add_argument("--model_path", type=str, default="models/sac_ball_model_online.pth", help="Path to the trained model")
    ...
    parser.add_argument("--plot_path", type=str, default="ball_trajectories.png", help="Path to save trajectory plot")
