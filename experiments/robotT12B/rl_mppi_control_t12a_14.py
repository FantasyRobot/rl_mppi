...
def main() -> None:
    ...
    default_sac = os.path.join(this_dir, "models", "sac_t12a_14_model.pth")

    p = argparse.ArgumentParser(
        description="RL-MPPI (SAC proposal + MPPI) control for t12a_14 (MuJoCo) from start to goal site"
    )
    p.add_argument("--xml", type=str, default=default_xml, help="Path to MJCF xml")
    p.add_argument("--sac_model", type=str, default=default_sac, help="Path to trained SAC checkpoint (.pth)")
    ...
    sac_path = os.path.expanduser(os.path.expandvars(str(args.sac_model)))
    if not os.path.isabs(sac_path):
        sac_path = os.path.abspath(sac_path)
    if not os.path.exists(sac_path):
        raise SystemExit(f"SAC model not found: {sac_path}")
    ...
    # Load model with CWD set to XML directory so mesh paths resolve.
    with _pushd(xml_dir):
        ...
        mujoco.mj_forward(model, data)
        goal_pos0 = np.asarray(data.site_xpos[goal_sid], dtype=np.float64).copy()

        policy = load_sac_policy(sac_path)
        ctrl = RLMuJoCoArmMPPI(
            model,
            policy,
        ...
        def step_once() -> tuple[float, np.ndarray, np.ndarray]:
            rep = max(1, int(args.action_repeat))
            u = None
            for _ in range(rep):
                if u is None:
                    u = ctrl.get_action(data.qpos, data.qvel)
                data.ctrl[:] = u
        ...
        with mujoco.viewer.launch_passive(model, data, key_callback=_key_callback) as viewer:
            ...
            # Show initial goal position for sanity.
            try:
                viewer.set_texts((None, None, "RL-MPPI running", f"goal={goal_pos0[0]:.2f},{goal_pos0[1]:.2f},{goal_pos0[2]:.2f}"))
            except Exception:
                pass
