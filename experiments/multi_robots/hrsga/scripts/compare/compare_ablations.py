...
from benchmark_policy import aggregate_runs, run_episode
from hrsga_model import load_agent_from_checkpoint


def evaluate_ablation(model_path, label, num_tests, seed, max_steps, layout_mode, strict_collision_stop, xml_path=None):
    agent = load_agent_from_checkpoint(model_path)
    runs = []
    for test_idx in range(num_tests):
        runs.append(run_episode("hrsga", agent, seed + test_idx, max_steps=max_steps, layout_mode=layout_mode, strict_collision_stop=strict_collision_stop, xml_path=xml_path))
