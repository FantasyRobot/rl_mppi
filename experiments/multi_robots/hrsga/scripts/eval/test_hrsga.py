...
from hrsga_model import load_agent_from_checkpoint
...
def test_hrsga(
    ...
    print(f"Testing HRSGA agent for {num_tests} runs with {layout_label} start/task layouts...")
    summary_runs = []
        ...
        summary_runs.append(
            {
                ...
                "missed_deadlines": float(info.get("missed_deadlines", 0.0)),
    ...
    if viewer_mode == "mujoco":
        for run_idx, run in enumerate(all_runs):
            ...
            visualize_rollout_in_mujoco(
                resolved_xml_path,
                run,
                title=f"HRSGA Run {run_idx + 1}/{len(all_runs)}",
...
def main():
    ...
    parser.add_argument("--model_path", type=str, default=os.path.join(ROOT_DIR, "models", "hrsga_best.pt"))
    ...
    parser.add_argument("--plot_path", type=str, default="hrsga_eval.png")
