...
DEFAULT_CONFIG_PATH = os.path.join(ROOT_DIR, "configs", "hrsga.json")

from env_hrsga import HRSGAEnvironment
from hrsga_model import HRSGAAgent, build_behavior_dataset, compute_action_loss, evaluate_agent, iterate_minibatches
...
def _build_run_paths(output_root, run_name):
    ...
    return {
        ...
        "best_path": os.path.join(checkpoint_dir, "hrsga_best.pt"),
        "latest_path": os.path.join(checkpoint_dir, "hrsga_latest.pt"),
        "canonical_best_path": os.path.join(ROOT_DIR, "models", "hrsga_best.pt"),
        "canonical_latest_path": os.path.join(ROOT_DIR, "models", "hrsga_latest.pt"),
    }
...
def _environment_summary(max_steps, xml_path=None, enforce_visit_order=None):
    env = HRSGAEnvironment(max_steps=max_steps, xml_path=xml_path, enforce_visit_order=enforce_visit_order)
    return {
        "xml_path": env.xml_path,
        "configured_num_robots": int(env.num_robots),
        "configured_num_tasks": int(env.num_tasks),
        "parsed_robot_templates": int(len(env.robot_start_templates)),
        "parsed_goal_templates": int(len(env.goal_position_templates)),
        "parsed_obstacles": int(len(env.obstacles)),
        "world_size": float(env.world_size),
        "max_steps": int(env.max_steps),
        "robot_radius": float(env.robot_radius),
        "task_radius": float(env.task_radius),
    }
...
def train(
    *,
    ...
    num_robots = int(environment_summary["configured_num_robots"])
    num_tasks = int(environment_summary["configured_num_tasks"])
    print(
        "[ENV] "
        f"xml={environment_summary['xml_path']} "
        f"robots={environment_summary['configured_num_robots']} "
        f"tasks={environment_summary['configured_num_tasks']} "
        f"obstacles={environment_summary['parsed_obstacles']} "
        f"robot_templates={environment_summary['parsed_robot_templates']} "
        f"goal_templates={environment_summary['parsed_goal_templates']} "
        f"robot_radius={environment_summary['robot_radius']:.3f} "
        f"task_radius={environment_summary['task_radius']:.3f} "
        f"world_size={environment_summary['world_size']:.2f} "
        f"max_steps={environment_summary['max_steps']}"
    )
    print(
        "[DATA] building expert dataset "
        f"episodes={train_episodes + val_episodes} "
        "source=reference_target_ik "
        "progress_logs=enabled"
    )
...
def main():
    parser = argparse.ArgumentParser(description="HRSGA imitation training for multi-robot task planning")
