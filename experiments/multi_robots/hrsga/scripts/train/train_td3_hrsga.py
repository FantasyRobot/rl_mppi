...
from hrsga_model import HRSGAPolicyNetwork, HRSGAQNetwork, StructuredReplayBuffer, snapshot_to_model_inputs
...
class TD3HRSGAAgent:
    ...
    def update(self, batch, policy_update=True):
        ...
        with torch.no_grad():
            ...
            next_actions = (next_actions + noise).clamp(-3.0, 3.0)
...
def load_agent_from_checkpoint(path):
    ...
    env = HRSGAEnvironment(xml_path=xml_path)
    agent = TD3HRSGAAgent(
        num_robots=int(env.num_robots),
        max_steps=int(checkpoint.get("max_steps", 280)),
...
def run_episode(env, agent, episode_idx, global_step, random_exploration_episodes, policy_sigma, policy_sigma_decay_rate):
    ...
    while True:
        if episode_idx < random_exploration_episodes:
            action = np.random.uniform(-1.0, 1.0, size=(env.num_robots, env.action_dim)).astype(np.float32) * 3.0
        else:
...
def evaluate_agent(agent, num_episodes=8):
    ...
    for episode_idx in range(num_episodes):
        env = HRSGAEnvironment(max_steps=agent.max_steps, xml_path=agent.xml_path)
...
def train(
    *,
    ...
    if resume_path:
        ...
        agent = TD3HRSGAAgent(num_robots=env.num_robots, max_steps=env.max_steps, xml_path=xml_path)
...
def main():
    parser = argparse.ArgumentParser(description="HRSGA multi-robot TD3 training")
