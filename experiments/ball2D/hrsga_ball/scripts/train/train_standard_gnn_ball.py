...
def train(
    *,
    ...
    dataset, episode_stats = build_behavior_dataset(num_episodes=train_episodes + val_episodes, num_balls=num_balls, max_steps=max_steps, seed_start=seed)
    ...
    agent = StandardGNNBallAgent(num_balls=num_balls, max_steps=max_steps, hidden_dim=hidden_dim, edge_hidden_dim=edge_hidden_dim)
    ...
    for epoch in range(start_epoch + 1, start_epoch + epochs + 1):
        ...
        if eval_interval > 0 and epoch % eval_interval == 0:
            metrics = evaluate_agent(agent, num_episodes=rollout_episodes, num_balls=num_balls, max_steps=max_steps, base_seed=seed + 10000)
