...
try:
    from .envball_hrsga import HRSGABallEnvironment, pairwise_distance, vec_add, vec_clip_norm, vec_mul, vec_norm, vec_sub, vec_unit
except ImportError:
    from envball_hrsga import HRSGABallEnvironment, pairwise_distance, vec_add, vec_clip_norm, vec_mul, vec_norm, vec_sub, vec_unit
...
class HRSGAExpertController:
    ...
    def _tr_score(self, snapshot, agent_index, agent, task):
        ...
        assigned = 1.0 if task["assigned_agent"] == agent_index else 0.0
    ...
    def _tr_message(self, snapshot, agent_index, agent, task):
        ...
        assigned = 1.0 if task["assigned_agent"] == agent_index else 0.35
    ...
    def _self_task_force(self, snapshot, agent_index, agent):
        task = snapshot["tasks"][agent_index]
        if task["completed"]:
            return (0.0, 0.0)
        rel = vec_sub(task["pos"], agent["pos"])
        direction = vec_unit(rel)
        release_wait = max(0, task["release_step"] - snapshot["step"])
        release_gate = 0.65 if release_wait > 0 else 1.0
    ...
    def _or_score(self, snapshot, agent, obstacle):
        ...
        dist = max(1e-6, vec_norm(rel))
        clearance = dist - (obstacle["radius"] + snapshot["ball_radius"])
        return 2.5 / (clearance + 0.35)

    def _or_message(self, snapshot, agent, obstacle):
        rel = vec_sub(agent["pos"], obstacle["center"])
        dist = max(1e-6, vec_norm(rel))
        clearance = obstacle["radius"] + snapshot["ball_radius"] + 0.9
        direction = vec_unit(rel)
    ...
    def act(self, snapshot):
        ...
        for agent_index, agent in enumerate(snapshot["agents"]):
            ...
            actions.append(vec_clip_norm(action, 1.4))
        return actions
...
def snapshot_to_sample(snapshot, max_steps, action_targets):
    ...
    ball_radius = float(snapshot["ball_radius"])
    ...
    for task_index, task in enumerate(tasks):
        ...
        task_features[task_index] = np.array(
            [
                ...
                _positive_clip(task["assigned_agent"], max(num_agents - 1, 1)),
    ...
    for agent_index, agent in enumerate(agents):
        own_task = tasks[agent["task_id"]]
        release_slack = own_task["release_step"] - step
        deadline_slack = own_task["deadline_step"] - step
        task_rel = vec_sub(own_task["pos"], agent["pos"])
        active_mask[agent_index] = not agent["reached"]
        robot_features[agent_index] = np.array(
            [
                ...
                _signed_clip(release_slack, max_steps),
                _signed_clip(deadline_slack, max_steps),
                float(own_task["priority"]),
                float(own_task["completed"]),
                float(agent["reached"]),
            ],
            dtype=np.float32,
        )
        ...
        for other_index, other in enumerate(agents):
            ...
            other_x_time = abs(other["pos"][0]) / max(0.25, abs(other["vel"][0]) + 0.25)
            conflict_gap = abs(own_x_time - other_x_time)
            yield_hint = 1.0 if tasks[other["task_id"]]["deadline_step"] < own_task["deadline_step"] else 0.0
        ...
        for task_index, task in enumerate(tasks):
            ...
            tr_edges[agent_index, task_index] = np.array(
                [
                    ...
                    _signed_clip(release_slack, max_steps),
                    _signed_clip(deadline_slack, max_steps),
                    float(task["assigned_agent"] == agent_index),
                    float(task["priority"]),
                    float(release_slack <= 0),
                ],
                dtype=np.float32,
        ...
        for obstacle_index, obstacle in enumerate(obstacles):
            ...
            dist = vec_norm(rel)
            clearance = dist - obstacle["radius"] - ball_radius
...
class HRSGAPolicyNetwork(nn.Module):
    def __init__(self, robot_dim, task_dim, obstacle_dim, rr_edge_dim, tr_edge_dim, or_edge_dim, hidden_dim=128, num_heads=4, topk_robot=2, topk_task=2, topk_obstacle=1, action_scale=1.4, disable_temporal_bias=False, disable_geometric_bias=False, shared_relation_attention=False, use_dense_residual=False):
        super().__init__()
...
class HRSGAQNetwork(nn.Module):
    def __init__(self, robot_dim, task_dim, obstacle_dim, rr_edge_dim, tr_edge_dim, or_edge_dim, hidden_dim=128, num_heads=4, topk_robot=2, topk_task=2, topk_obstacle=1, disable_temporal_bias=False, disable_geometric_bias=False, shared_relation_attention=False, use_dense_residual=False):
        ...
        self.action_encoder = MLP(hidden_dim + 2, [hidden_dim, hidden_dim], hidden_dim)
...
class HRSGABallAgent:
    def __init__(self, num_balls, max_steps, hidden_dim=128, num_heads=4, topk_robot=2, topk_task=2, topk_obstacle=1, disable_temporal_bias=False, disable_geometric_bias=False, shared_relation_attention=False, use_dense_residual=False, device=None):
    ...
    def select_action(self, snapshot):
        sample = snapshot_to_sample(snapshot, max_steps=self.max_steps, action_targets=np.zeros((self.num_balls, 2), dtype=np.float32))
...
def evaluate_agent(agent, num_episodes=10, num_balls=None, max_steps=None, base_seed=5300, layout_mode="structured", strict_collision_stop=False):
