...
class HRSGABallAgent:
    def __init__(self, num_balls, max_steps, num_tasks=None, hidden_dim=128, num_heads=4, topk_robot=2, topk_task=2, topk_obstacle=1, disable_temporal_bias=False, disable_geometric_bias=False, shared_relation_attention=False, use_dense_residual=False, device=None):
    ...
    def select_action(self, snapshot):
        sample = snapshot_to_sample(snapshot, max_steps=self.max_steps, action_targets=np.zeros((len(snapshot["agents"]), 2), dtype=np.float32))
...
def evaluate_agent(agent, num_episodes=10, num_balls=None, num_tasks=None, max_steps=None, base_seed=5300, layout_mode="structured", strict_collision_stop=False):
