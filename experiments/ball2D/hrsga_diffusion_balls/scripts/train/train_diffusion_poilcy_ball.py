...
from diffusion_ball_model import (
...
def _build_run_paths(output_root, run_name):
    ...
    resolved_run_name = run_name or f"diffusion_poilcy_run_{timestamp}"
    ...
    return {
        ...
        "best_path": os.path.join(checkpoint_dir, "hrsga_diffusion_poilcy_best.pt"),
        "latest_path": os.path.join(checkpoint_dir, "hrsga_diffusion_poilcy_latest.pt"),
        "canonical_best_path": os.path.join(ROOT_DIR, "models", "hrsga_diffusion_poilcy_best.pt"),
        "canonical_latest_path": os.path.join(ROOT_DIR, "models", "hrsga_diffusion_poilcy_latest.pt"),
    }
...
def train(
    *,
    ...
    dataset, episode_stats, dataset_metadata = build_diffusion_behavior_dataset(
    ...
    train_dataset, val_dataset = _split_diffusion_dataset(dataset, train_episodes / max(train_episodes + val_episodes, 1))
    dataset_summary = {
        ...
        "horizon": int(dataset_metadata.get("horizon", horizon)),
    }
    ...
    teacher = HRSGADiffusionPoilcyAgent(
        num_balls=num_balls,
        num_tasks=num_tasks,
        max_steps=max_steps,
        horizon=horizon,
    ...
    optimizer = torch.optim.AdamW(teacher.model.parameters(), lr=learning_rate, weight_decay=weight_decay)

    start_epoch = 0
    best_score = -np.inf
    if resume_path:
        metadata = teacher.load(resume_path, optimizer=optimizer)
    ...
    for epoch in range(start_epoch + 1, start_epoch + epochs + 1):
        teacher.model.train()
        train_losses = []
        for batch in iterate_diffusion_minibatches(train_dataset, batch_size=batch_size, shuffle=True, seed=seed + epoch):
            optimizer.zero_grad()
            loss = teacher.diffusion_loss(batch)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(teacher.model.parameters(), max_norm=2.0)
            optimizer.step()
        ...
        val_losses = []
        with torch.no_grad():
            for batch in iterate_diffusion_minibatches(val_dataset, batch_size=batch_size, shuffle=False):
                loss = teacher.diffusion_loss(batch)
        ...
        if eval_interval > 0 and epoch % eval_interval == 0:
            metrics = evaluate_diffusion_poilcy(
                teacher,
            ...
            if score > best_score:
                ...
                _safe_save_checkpoint(teacher, run_paths["best_path"], epoch=epoch, best_score=best_score, optimizer=optimizer, required=True)
                _safe_save_checkpoint(teacher, run_paths["canonical_best_path"], epoch=epoch, best_score=best_score, optimizer=optimizer, required=False)
        ...
        if save_interval > 0 and epoch % save_interval == 0:
            _safe_save_checkpoint(teacher, run_paths["latest_path"], epoch=epoch, best_score=best_score, optimizer=optimizer, required=True)
            _safe_save_checkpoint(teacher, run_paths["canonical_latest_path"], epoch=epoch, best_score=best_score, optimizer=optimizer, required=False)
            print(f"[SAVE] epoch={epoch} latest checkpoint saved to {run_paths['latest_path']}")

    _safe_save_checkpoint(teacher, run_paths["latest_path"], epoch=start_epoch + epochs, best_score=best_score, optimizer=optimizer, required=True)
    _safe_save_checkpoint(teacher, run_paths["canonical_latest_path"], epoch=start_epoch + epochs, best_score=best_score, optimizer=optimizer, required=False)
    _save_json(
    ...
    print("Diffusion poilcy 训练完成，模型已保存。")


def main():
    parser = argparse.ArgumentParser(description="Train a graph-conditioned diffusion poilcy for the multi-ball HRSGA task")
    ...
    parser.add_argument("--diffusion_steps", type=int, default=16)
