...
def _resolve_default_model_path(model_type):
    ...
    default_names = {
        "hrsga": "hrsga_ball_best.pt",
        "standard_gnn": "standard_gnn_ball_best.pt",
        "diffusion_poilcy": "hrsga_diffusion_poilcy_best.pt",
...
def load_agent(model_type, model_path):
    ...
    if model_type == "diffusion_poilcy":
        return load_diffusion_poilcy_from_checkpoint(model_path)
    if model_type == "td3":
...
def select_action(model_type, agent, snapshot):
    ...
    if model_type == "diffusion_poilcy":
        return agent.select_action(snapshot, temperature=0.0)
    return agent.select_action(snapshot)


def run_episode(model_type, agent, seed, max_steps=None, layout_mode="representative", strict_collision_stop=False, num_balls=None, num_tasks=None):
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark a trained policy checkpoint on the ball environment")
    parser.add_argument("--model_type", type=str, choices=["hrsga", "standard_gnn", "diffusion_poilcy", "td3"], required=True)
