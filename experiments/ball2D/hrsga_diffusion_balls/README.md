# hrsga_diffusion_balls

该目录用于验证 HRSGA 在二维多机器人多目标遍历与避障场景中的训练、测试和对比流程。若按论文主方法划分，当前核心是两阶段：第一阶段 HRSGA 学习，第二阶段图条件 diffusion model 学习并直接作为最终策略评测；此外仓库还保留 TD3-HRSGA 与 Standard GNN 两条辅助对比主线。当前整理为四条主线：

- HRSGA 监督学习主线。
- Diffusion 主线，用于学习短时动作片段分布并作为第二阶段生成式策略评测。
- TD3-HRSGA 强化学习主线。
...
说明：当前仓库已经统一使用 `diffusion_poilcy` 命名。这里的第二阶段指 graph-conditioned diffusion model 直接作为最终策略评测，而不是额外的第三阶段 teacher-to-student 管线。
...
- `diffusion_ball_model.py`
  - 第二阶段 diffusion model、短时动作片段数据集构造与直接策略 rollout 评估工具。
...
- `train_diffusion_poilcy_ball.py`
  - 第二阶段 graph-conditioned diffusion model 训练入口。
...
- `train_diffusion_poilcy_ball.example.json`
  - Diffusion poilcy 训练配置示例，共享训练参数与 `train_hrsga_ball.example.json` 对齐，并额外补充 `horizon`、`diffusion_steps` 与 `sample_temperature`。
...
- `benchmark_policy_ball.py`
  - 通用正式评估脚本，可对 HRSGA、Standard GNN、diffusion model、TD3 进行多随机种子统计。
...
- `models/`
  ...
  - 现保留：
    ...
    - `hrsga_diffusion_poilcy_best.pt`
    - `hrsga_diffusion_poilcy_latest.pt`
...
1. 第一阶段训练 HRSGA：`scripts/train/train_hrsga_ball.py`
2. 第二阶段训练 diffusion model：`scripts/train/train_diffusion_poilcy_ball.py`
3. 训练 Standard GNN 基线：`scripts/train/train_standard_gnn_ball.py`
4. 训练 TD3-HRSGA：`scripts/train/train_td3_hrsga_ball.py`
5. 运行单模型测试或生成轨迹图：`scripts/eval/test_hrsga_ball.py`、`scripts/eval/test_td3_hrsga_ball.py`
6. 生成正式统计结果：`scripts/eval/benchmark_policy_ball.py`
7. 比较 HRSGA 与 Standard GNN：`scripts/compare/compare_trained_gnn_hrsga_ball.py`、`scripts/compare/compare_policy_metrics_ball.py`
8. 比较 HRSGA 消融版本：`scripts/train/train_ablation_hrsga_ball.py`、`scripts/compare/compare_ablations_ball.py`
9. 绘制训练曲线：`scripts/plot/plot_training_curves_ball.py`
10. 查看论文与实验计划：`docs/paper/draft.md`、`docs/paper/experiment_plan.md`
...
当前 `scripts/train/train_diffusion_poilcy_ball.example.json` 也采用与 HRSGA 示例一致的共享训练参数，并同样默认启用 `"expert_max_collisions": 0`；在此基础上增加了第二阶段 diffusion model 所需的 `"horizon"`、`"diffusion_steps"` 和 `"sample_temperature"` 字段。
...
```powershell
cd d:\VirtualSpace\rl_mppi\experiments\ball2D\hrsga_diffusion_balls
```
...
后文所有命令都默认从 `hrsga_diffusion_balls` 根目录执行。

### 配置文件与命令行覆盖规则

- `train_hrsga_ball.py`、`train_diffusion_poilcy_ball.py`、`train_standard_gnn_ball.py`、`train_ablation_hrsga_ball.py` 支持 `--config_path`。
...
### 1. 第二阶段训练 diffusion model

```powershell
D:/python3.12/python.exe scripts/train/train_diffusion_poilcy_ball.py --config_path scripts/train/train_diffusion_poilcy_ball.example.json --run_name diffusion_poilcy_formal
```

常用变体：

```powershell
D:/python3.12/python.exe scripts/train/train_diffusion_poilcy_ball.py --config_path scripts/train/train_diffusion_poilcy_ball.example.json --run_name diffusion_poilcy_2ball_8task --num_balls 2 --num_tasks 8
```

核心参数补充：

- `--config_path`：训练配置 JSON 路径。当前示例文件与 `train_hrsga_ball.example.json` 保持同一套共享训练超参数，只额外补充 diffusion model 特有字段。
- `--horizon`：模型预测的短时动作片段长度。
- `--diffusion_steps`：反向扩散采样步数。
- `--sample_temperature`：直接策略 rollout 时的采样温度；`0.0` 表示更稳定的确定性生成策略。

### 1.1 正式评测第二阶段 diffusion model

```powershell
D:/python3.12/python.exe scripts/eval/benchmark_diffusion_poilcy_formal.py --model_path runs/formal_diffusion_poilcy_4r8t/checkpoints/hrsga_diffusion_poilcy_best.pt --strict_collision_stop
```

该命令会把第二阶段 diffusion model 直接作为独立策略，在 `4r8t`、`2r8t` 和 `2r4t` 三组 representative 设置下各执行 20 次测试，并把汇总结果写入 `runs/benchmarks/diffusion_poilcy_formal.json`。
